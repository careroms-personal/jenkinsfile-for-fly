...
- **Mismatch to verify:** a generated Jenkinsfile pipeline passes `-var "ami_id=$AMI_ID"` conditionally when the customer YAML specifies one, but per the current spec `ami_id` is not a declared module/caller variable (it's an internal `aws_ami` data source). If `environments/uat/variables.tf` doesn't declare `ami_id`, that `-var` flag is silently ignored by OpenTofu (undeclared `-var` = warning, not error) rather than actually overriding the AMI. Before relying on per-customer AMI overrides working, confirm whether `ami_id` was actually added as a real variable with the data source as its fallback default — don't assume the Jenkinsfile and the `.tf` files agree on this without checking.
